#include <iostream>
#include <string>

using namespace std;

int main()
{
	cout << "The size of type int is " << sizeof(int) << " bytes." << endl;
	cout << "The size of type short is " << sizeof(short) << " bytes." << endl;
	cout << "The size of type long is " << sizeof(long) << " bytes." << endl;
	cout << "The size of type float is " << sizeof(float) << " bytes." << endl;
	cout << "The size of type double is " << sizeof(char) << " bytes." << endl;
	cout << "The size of type bool is " << sizeof(bool) << " bytes." << endl;
	cout << "The size of type string is " << sizeof(string) << " bytes." << endl;
	system("pause");
}